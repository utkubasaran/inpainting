...
def auto_detect_masks(
    image: Image.Image,
    model_name: str,
) -> Tuple[Image.Image, Image.Image, EditorValue, EditorValue, str]:
    if gr is None:
...
def apply_recolor(
    ...
    result = recolor_region(working, skirt_mask, hex_to_rgb(skirt_color), strength=strength)
...
def build_demo() -> gr.Blocks:
    ...
    with gr.Blocks(title="Clothing Recolor for Colab") as demo:
        gr.Markdown(
            """
            # Clothing Recolor
            Upload a photo, auto-detect skirt and socks, make small mask corrections if needed,
        ...
        detect_button.click(
            ...
            outputs=[skirt_overlay, socks_overlay, skirt_editor, socks_editor, status_text],
        )
        apply_button.click(
            fn=apply_recolor,
            inputs=[image_input, skirt_editor, socks_editor, skirt_color, socks_color, strength],
